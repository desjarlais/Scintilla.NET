...
public class Marker
{
    ...
    public unsafe void DefineRgbaImage(Bitmap image)
    {
        ...
        this.scintilla.DirectMessage(SciApi.SCI_RGBAIMAGESETWIDTH, new IntPtr(image.Width));
        this.scintilla.DirectMessage(SciApi.SCI_RGBAIMAGESETHEIGHT, new IntPtr(image.Height));

        byte[] bytes = Helpers.BitmapToArgb(image);
        fixed (byte* bp = bytes)
            this.scintilla.DirectMessage(SciApi.SCI_MARKERDEFINERGBAIMAGE, new IntPtr(Index), new IntPtr(bp));
    }
    ...
    public void SetAlpha(int alpha)
    {
        ...
        this.scintilla.DirectMessage(SciApi.SCI_MARKERSETALPHA, new IntPtr(Index), new IntPtr(alpha));
    }
    ...
    public void SetBackColor(Color color)
    {
        ...
        this.scintilla.DirectMessage(SciApi.SCI_MARKERSETBACKTRANSLUCENT, new IntPtr(Index), new IntPtr(colour));
    }
    ...
    {
        int colour = HelperMethods.ToWin32Color(color);
        this.scintilla.DirectMessage(SciApi.SCI_MARKERSETFORETRANSLUCENT, new IntPtr(Index), new IntPtr(colour));
    }
    ...
    public MarkerSymbol Symbol
    {
        get
        {
            return (MarkerSymbol)this.scintilla.DirectMessage(SciApi.SCI_MARKERSYMBOLDEFINED, new IntPtr(Index));
        }
        set
        {
            int markerSymbol = (int)value;
            this.scintilla.DirectMessage(SciApi.SCI_MARKERDEFINE, new IntPtr(Index), new IntPtr(markerSymbol));
        }
    }
