...
internal static class AuthenticodeHelper
{
    ...
    /// </summary>
    /// <param name="filePath">The full path to the file to verify.</param>
    /// <param name="expectedSubjectFragment">
    /// A fragment of the certificate Subject (e.g., "O=MyCompany") to match, or <c>null</c> to skip publisher validation.
    /// </param>
    /// <returns><c>true</c> if the signature is valid and the publisher matches (if specified); otherwise, <c>false</c>.</returns>
    public static bool VerifySignature(string filePath, string expectedSubjectFragment = null)
    {
        if (!HasValidSignature(filePath))
            return false;

        if (string.IsNullOrEmpty(expectedSubjectFragment))
            return true;
        ...
        try
        {
            ...
            return cert.Subject != null &&
                   cert.Subject.IndexOf(expectedSubjectFragment, StringComparison.OrdinalIgnoreCase) >= 0;
        }
        catch
        {
            return false;
